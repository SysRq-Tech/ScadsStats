...
class DatePicker(TextInput):
    ...
    def __init__(self, size_hint=(0.5, 0.7), touch_switch=False, *args, **kwargs):
        super(DatePicker, self).__init__(*args, **kwargs)
        self.popup_hint = size_hint
    ...
    def init_ui(self):
        ...
        self.popup = Popup(content=self.cal, on_dismiss=self.update_value, 
                           title="", size_hint=self.popup_hint)
        self.cal.parent_popup = self.popup
        
        self.bind(on_double_tap=self.show_popup)
...
class CalendarWidget(RelativeLayout):
    ...
    def create_month_scr(self, month, toggle_today=False):
        ...
        scr = Screen()
        m = self.month_names[self.active_date[1] - 1]
        ...
        for i in range(7):
            if i >= 5:  # weekends
                l = DayAbbrWeekendLabel(text=self.days_abbrs[i])
            else:  # work days
                l = DayAbbrLabel(text=self.days_abbrs[i])
    ...
    def prepare_data(self):
        ...
        self.days_abbrs = cal_data.get_days_abbrs()
    ...
    def go_prev(self, inst):
        ...
        prev_scr_name = "%s-%s" % (self.month_names[n],
                                   self.quarter_nums[0][0])
        
        # If it's doesn't exist, create it
    ...
    def go_next(self, inst):
        ...
        next_scr_name = "%s-%s" % (self.month_names[n],
                                   self.quarter_nums[2][0])
